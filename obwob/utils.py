...
from .models import Event, Facilitator, Participant, Question
...
def get_model(model_name):
    return apps.get_model('obwob', model_name)
...
def use_custom_serializer(model_name, serializers_dict):
    if model_name == 'EventParticipant':
        return create_event_participant_serializer() 
    elif model_name == 'Event':
        return create_event_serializer(serializers_dict)
    
    
def use_custom_viewset(model_name, serializers_dict):
    if model_name == 'EventParticipant':
        return create_event_participant_viewset()
    elif model_name == 'Event':
        return create_event_viewset(serializers_dict)
    

# custom serializer functions 
def create_event_participant_serializer():
...
def create_event_serializer(serializers_dict):
...
def create_event_participant_viewset():
    class EventParticipantViewSet(viewsets.ModelViewSet):
        queryset = Event.objects.all()
        serializer_class = create_event_participant_serializer()

    return EventParticipantViewSet


def create_event_viewset(serializers_dict):
    class EventViewSet(viewsets.ModelViewSet):
        queryset = Event.objects.all()
        serializer_class = create_event_serializer(serializers_dict)
